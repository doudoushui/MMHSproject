...
<body>
  <?php  var_dump($_SESSION); $AUTH = new \Think\Auth(); var_dump($AUTH->getGroups(session('adminid'))); var_dump($AUTH->getAuthList(session('adminid'))); ?>
    <div id="wrapper">
                ...
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                           <a href="#"><i class="fa fa-bar-chart-o fa-fw"></i> 用户管理<span class="fa arrow"></span></a>
                            <ul class="nav nav-second-level">
                               <?php  $AUTH = new \Think\Auth(); var_dump($AUTH->getGroups(session('adminid'))); die(); if($AUTH->check('Admin/User/index', session('adminid'))){ ?>
                                  <li>
                                    <a href="<?php echo U('Admin/User/index');?>">用户列表</a>
                                 </li>
                                <?php  } ?>
                            
                                    <?php  $AUTH = new \Think\Auth(); if($AUTH->check('Admin/User/add', session('adminid'))){ ?>
                                    <li>
                        ...
                        <li>
                            ...
                            <ul class="nav nav-second-level">
                            <?php  $AUTH = new \Think\Auth(); if($AUTH->check('Admin/Adminuser/index', session('adminid'))){ ?>
                                <li>
                                    <a href="<?php echo U('Admin/Adminuser/index');?>">管理员列表</a>
                                </li>
                                <?php  } ?>
                                  <?php  $AUTH = new \Think\Auth(); if(!$AUTH->check('Admin/Adminuser/add', session('adminid'))){ ?>
                                <li>
                        ...
                        <?php  $AUTH = new \Think\Auth(); echo MODULE_NAME; echo CONTROLLER_NAME; echo ACTION_NAME; if($AUTH->check(MODULE_NAME.'/'.CONTROLLER_NAME.'/'.ACTION_NAME, session('uid'))){ ?>
   ...
   <div class="panel panel-default">
    <div class="panel-heading">
     ...
     <div class="panel-body">
      <div class="row">
       <div class="col-lg-6">
        <form role="form" action="<?php echo U('Admin/Cate/insert');?>" method="post" enctype="multipart/form-data">
         <div class="form-group">
           <label>所属分类列表</label>
           <!-- <select name="pid" class="form-control">
               <option value="0">顶级分类</option>
              <?php if(is_array($cates)): foreach($cates as $k=>$vo): ?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["cname"]); ?></option><?php endforeach; endif; ?>
           </select> -->
            <select name="pid" class="form-control">
                 <option value="0">顶级分类</option>
                <?php if(is_array($cates)): foreach($cates as $k=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo["id"] == $info['pid'] ): ?>selected<?php endif; ?> ><?php echo ($vo["cname"]); ?></option><?php endforeach; endif; ?>
             </select>
         </div>
         <div class="form-group">
             <label>分类名称</label>
             <input name="cname" placeholder="请输入恰当的商品分类名哟" class="form-control">
         </div>
