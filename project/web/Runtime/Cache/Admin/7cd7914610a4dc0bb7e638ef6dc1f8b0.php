                ...
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                            ...
                            <ul class="nav nav-second-level">
                                <li>
                                    <a href="<?php echo U('Admin/User/index');?>">评论列表</a>
                                </li>
                                <li>
                                    <a href="<?php echo U('Admin/User/add');?>">评论添加</a>
                                </li>
                            </ul>
                            <!-- /.nav-second-level -->
                        </li>
                        ...
                        <li>
                            ...
                            <ul class="nav nav-second-level">
                                <li>
                                    <a href="<?php echo U('Admin/Recomment/index');?>">婚庆推荐列表</a>
                                </li>
                                <li>
                                    <a href="<?php echo U('Admin/Recomment/add');?>">婚庆推荐添加</a>
                                </li>
                            </ul>
                            <!-- /.nav-second-level -->
                        </li>
                ...
                <div class="row">
                    ...
                    <div class="dataTables_length" id="dataTables-example_length">
                      <label>显示 
                        <select name="num" aria-controls="dataTables-example" class="form-control input-sm">
                          ...
                          <option <?php echo $_GET['num'] == 5 ? 'selected' : '' ?> value="5" selected>5</option>
                  ...
                  <div class="col-sm-6">
                    <div id="dataTables-example_filter" class="dataTables_filter">
                      <label>分类名:<input value="<?php echo ($_GET['keyword']); ?>" name="keyword" type="search" class="form-control input-sm" placeholder="" aria-controls="dataTables-example"></label>
                      <button type="submit" class="btn btn-info glyphicon glyphicon-search"> 查询</button>
                    </div>
                  </div>
                    ...
                    <table id="dataTables-example" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dataTables-example_info">
                      <thead>
                        <tr role="row">
                          ...
                          <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 129px;" aria-label="Browser: activate to sort column ascending">分类名</th>
                          <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 124px;" aria-label="Platform(s): activate to sort column ascending">所属分类</th>
                          <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 98px;" aria-label="Engine version: activate to sort column ascending">path</th>
                          <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 98px;" aria-label="Engine version: activate to sort column ascending">显示否</th>
                      ...
                      <tbody>
                        <?php if(is_array($cates)): foreach($cates as $k=>$vo): ?><tr class="gradeA odd" role="row">
                            <td class="sorting_1 sid"><?php echo ($vo["id"]); ?></td>
                            
                            <td class="edit"><?php echo ($vo["cname"]); ?></td>
                            <td>
                             <?php echo ($vo["pid"]); ?>
                            </td>
                            <td class="center"><?php echo ($vo["path"]); ?></td>
                            <td class="edit_display"><?php echo ($vo['display']?'显示':'不显示'); ?></td>
                            <td class="center">
                              <button class="btn btn-danger btn-del  btn-primary" type="button"><i class="fa fa-times"></i> 删除</button>
                              <a href="<?php echo U('Admin/Cate/save',array('id'=>$vo['id']));?>" class="btn btn-primary btn-primary" type="button"><i class="fa fa-wrench"></i> 修改</a>
                            </td>
  ...
  <script type="text/javascript">
   ...
   // <?php echo ($info['state']?$info['0']:$info['1']); ?>
    ...
    //获取id
      ...
      var url = "<?php echo U('Admin/Cate/ajaxedit_display');?>";
      // alert(url);
      var btn = $(this);
      //ajax
      $.post(url,{id:id,display:display},function(data){
        // console.log(data);
   ...
   $('.edit').dblclick(function(){
    ...
    var v = $(this).html();
    // 创建inp
    var inp = $('<input type="text" value="'+v+'" name="cname" />')
    //将input插入td中
    $(this).html(inp);
    //让input处于选中状态
    inp.select();
    ...
    inp.blur(function(){
      ...
      //获取当前的id
      var id = $(this).parents('tr').find('.sid').html();
      var url = "<?php echo U('Admin/Cate/ajaxedit');?>";
      //发送ajax
      $.get(url,{id:id,cname:newV},function(data){
        if(data == 1){
          // alert('修改成功');
          td.html(newV);
        }else{
